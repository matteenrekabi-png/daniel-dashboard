...
const statusStyle = (status: string) => {
  if (status === 'confirmed') return { background: '#1e3a5f', color: '#60a5fa', border: '1px solid #2563eb44', animation: 'badgePop 0.35s ease both' }
  if (status === 'cancelled' || status === 'canceled') return { background: '#2a1a1a', color: '#f87171', border: '1px solid #7f1d1d44', animation: 'badgePop 0.35s ease both' }
  if (status === 'rescheduled') return { background: '#2a1f0a', color: '#fbbf24', border: '1px solid #78350f44', animation: 'badgePop 0.35s ease both' }
  return { background: '#1a1a1a', color: '#888888', border: '1px solid #2a2a2a', animation: 'badgePop 0.35s ease both' }
}
...
export default async function DashboardPage() {
  ...
  // Hidden calls — needed to match Call Logs count
  ...
  for (const c of calls.slice(0, 15)) {
    ...
    activity.push({
      ts: c.startedAt,
      label: 'Incoming call',
      sub: dur ? `${dur} call` : 'Call received',
      dot: '#2563eb',
    })
  }
  ...
  for (const a of apptList.slice(0, 15)) {
    ...
    const dateStr = a.appointment_date
      ...
      : ''
    const sub = `${name} — ${svc}${dateStr}`

    // Always record the booking event at created_at
    activity.push({
      ts: a.created_at,
    ...
    // If the appointment was later changed, add a second event
  ...
  return (
    <>
      <style>{`
        ...
        .stat-card { transition: border-color 0.2s ease, box-shadow 0.2s ease; }
        .stat-card:hover { border-color: #2a2a2a !important; box-shadow: 0 4px 24px rgba(0,0,0,0.3); }
        .panel-card { transition: border-color 0.2s ease, box-shadow 0.2s ease; }
        .panel-card:hover { border-color: #222 !important; }
        .activity-dot { transition: box-shadow 0.2s ease; }
        .convo-row { transition: background 0.15s ease; border-bottom: 1px solid #141414; }
        .convo-row:last-child { border-bottom: none; }
        .convo-row:hover { background: #111 !important; }
        .appt-row { transition: background 0.15s ease; border-bottom: 1px solid #141414; }
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            <div
              ...
              style={{ ...card, animation: `statCardIn 0.3s ease both`, animationDelay: stat.delay }}
            >
          ...
          <div
            className="stat-card p-6"
            style={{ ...card, animation: 'statCardIn 0.3s ease both', animationDelay: '0.15s' }}
          >
            ...
            {client?.vapi_assistant_id ? (
              <span
                ...
                style={{ background: '#0f2a1a', color: '#34d399', border: '1px solid #065f46', animation: 'badgePop 0.4s ease both', animationDelay: '0.25s' }}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div
            ...
            style={{ ...card, animation: 'statCardIn 0.35s ease both', animationDelay: '0.1s' }}
          >
            ...
            {!recentConversations.length ? (
              <div className="py-6 text-center space-y-1">
                ...
                <p className="text-xs" style={{ color: '#333' }}>Conversations will appear here once your agent goes live.</p>
              </div>
            ) : (
              <div>
                {recentConversations.map((call, idx) => {
                  ...
                  return (
                    <div
                      key={call.id}
                      className="convo-row py-3 px-2 rounded-lg"
                      style={{ animation: 'rowIn 0.25s ease both', animationDelay: `${0.05 * idx}s` }}
                    >
                      <div className="flex items-center justify-between gap-2 mb-1">
                        ...
                        {dur && (
                          <span
                            ...
                            style={{ background: '#1a1a1a', color: '#666', border: '1px solid #2a2a2a', animation: 'badgePop 0.3s ease both', animationDelay: `${0.1 + 0.05 * idx}s` }}
                          >
                            {dur}
                          </span>
                        )}
                      ...
                      <p
                        ...
                        style={{ color: hasSum ? '#888' : '#383838', fontStyle: hasSum ? 'normal' : 'italic', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' } as React.CSSProperties}
                      >
            ...
            {!recentAppts.length ? (
              ...
              <div>
                {recentAppts.map((appt, idx) => (
                  <div
                    key={appt.id}
                    className="appt-row flex items-start justify-between gap-2 py-3 px-2 rounded-lg"
                    style={{ animation: 'rowIn 0.25s ease both', animationDelay: `${0.05 * idx}s` }}
                  >
                    ...
                    <span
                      ...
                      style={{ ...statusStyle(appt.status), animationDelay: `${0.1 + 0.05 * idx}s` }}
                    >
                      {appt.status}
                    </span>
                  </div>
          ...
          <div
            className="panel-card p-6 lg:col-span-1"
            style={{ ...card, animation: 'statCardIn 0.35s ease both', animationDelay: '0.2s' }}
          >
              ...
              <div className="space-y-3">
                {activityFeed.map((item, i) => (
                  <div
                    ...
                    style={{ animation: 'rowIn 0.2s ease both', animationDelay: `${0.03 * i}s` }}
                  >
                    <div
                      className="activity-dot mt-1.5 shrink-0"
                      style={{
                        width: 7, height: 7, borderRadius: '50%', background: item.dot,
                        boxShadow: `0 0 6px ${item.dot}88`,
                        flexShrink: 0,
                      }}
                    />
                    <div className="min-w-0">
